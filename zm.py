...
base_repos = [
  'https://github.com/jangaraj/zabbix-manager/'
]
configs = [
  '/etc/zabbix/zabbix_manager.conf',
  '/usr/local/etc/zabbix/zabbix_manager.conf',  
]
default = {
  'verbose': 10,
} 
...
def configure_log_file(conf):
    ...
    if hasattr(conf, 'log_file_path') and conf.log_file_path.strip() != "":
    ...
    if hasattr(conf, 'syslog') and conf.syslog == False:
...
def buildParams(cfg, parser):
    ...
    parser.add_option('-a', '--all', action="store_true", dest='allsearch',
        help='search also in Zabbix community repos and Zabbix Share, Default is disabled', default=defcfg.get("allsearch", "False") == "True")
    
    parser.add_option('-v', '--verbose', dest='verbose', type="int", help='Verbose level - 0 default is None', default=defcfg.get("verbose", 0))

    return parser

def readCommadLine(config_file, arguments, usage):
...
def runMain(arguments, output=sys.stdout):
    ...
    for cfile in configs:
        if os.path.exists(cfile):
           conf = readCommadLine(cfile, arguments, usage)
           break
    
    # apply default values       
    if conf == None:
        conf = {}
        conf['verbose'] = default['verbose']            
    ...
    if 'search' in sys.argv: 
        log.debug("Search")
        if '-a' in sys.argv:
            log.debug("-a Github search - https://api.github.com/repos/zabbix/zabbix-community-repos/git/trees/master")
            s = requests.get('https://api.github.com/repos/zabbix/zabbix-community-repos/git/trees/master')
            s = json.loads(s.text)
            sstring = sys.argv
            sstring.remove('./zm.py')
            sstring.remove('search')
            for sparam in sstring:
                if sparam.startswith('-'):
                    sstring.remove(sparam)                    
            sstring = sstring[0]
            log.debug("Search string: " + sstring)
            result = 1
            for ofile in s['tree']:                
                if ofile['type'] == 'tree' and sstring.lower() in ofile['path'].lower():
                    print str(result) + '. ' + ofile['path'] + ' - https://github.com/zabbix/zabbix-community-repos/tree/master/' + ofile['path']
                    result += 1 
                    
    # agent action
    if 'installa' in sys.argv: 
        log.debug("Agent action")

    # server action
    if 'installs' in sys.argv: 
        log.debug("Server action")
